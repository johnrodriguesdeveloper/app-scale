...
export function useAvailability() {
  ...
  const [expandedCalendar, setExpandedCalendar] = useState<ExpandedCalendarItem[]>([])
  ...
  const loadMonthExceptions = useCallback(async () => {
    ...
    if (data) setMonthExceptions(data as AvailabilityException[])
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [currentMonth])
  ...
  const handleToggleException = async (item: ExpandedCalendarItem, newValue: boolean) => {
