...
import type { OnboardingDepartment, OnboardingFunctionItem } from "@/types/onboarding"
...
export function useOnboarding() {
  ...
  const [selectedSubDepartment, setSelectedSubDepartment] = useState<OnboardingDepartment | null>(
    null
  )
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      const functionsToInsert = selectedFunctions.map((funcId) => ({
        member_id: memberData.id as string,
        function_id: funcId,
      }))
      ...
      showModal("error", "Erro", error instanceof Error ? error.message : "Falha ao salvar dados.")
    } finally {
      setSaving(false)
    }
  }
